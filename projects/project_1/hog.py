...
def roll_dice(num_rolls, dice=six_sided):
    ...
    if 1 in points:     # Pig Out rule
        return 1    
    return sum(points)


def digits(x):
    '''Return a list of the first and second digits of a number up to 99.'''
    assert x >= 0 and x < 100 , 'number must be between 0 and 99'
    return [x // 10, x % 10]
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls == 0:      # Free Bacon rule
        return max(digits(opponent_score)) + 1
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    def won():
        nonlocal score, opponent_score
        return score >= GOAL_SCORE or opponent_score >= GOAL_SCORE
    def player_move(who):
        nonlocal score, opponent_score
        if who == 0:
            # Player 0 rolls the dice
            num_rolls = strategy0(score, opponent_score)
            dice_type = select_dice(score, opponent_score)
            score += take_turn(num_rolls, opponent_score, dice_type)
        else:
            # Player 1 rolls the dice
            num_rolls = strategy1(opponent_score,score)
            dice_type = select_dice(opponent_score, score)
            opponent_score += take_turn(num_rolls, score, dice_type)
    while not won():
        '''Main loop of the game.'''
        player_move(who)
        swine_swap()    # Swine Swap rule
        who = other(who)
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    s = []
    for i in range(1, 11):
        average = make_averaged(roll_dice, 1000)(i, dice)
        s.append(average)
        print(f'{i} dice scores {average} on average')
...
def swap_strategy(score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    hypo_score = score + (1 + max(digits(opponent_score)))  # Hypothetical score resulting from num_rolls = 0.
    if can_swap(hypo_score, opponent_score):
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    This strategy rolls 0 dice when it would result in a total score divisible by 7.
    This would cause the opposing player to roll 4-sided dice.
    """
    "*** YOUR CODE HERE ***"
    if swap_strategy(score, opponent_score) == 0:   # Implement swap strategy first to account for any swaps
        return 0                                    # that would result from this function returning 0.
    if bacon_strategy(score, opponent_score) == 0:
        return 0
    hypo_score = score + (1 + max(digits(opponent_score)))                                            
    if (hypo_score + opponent_score) % 7 == 0:
        return 0
    return BASELINE_NUM_ROLLS 
